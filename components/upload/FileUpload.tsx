    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header>
        ...
        <p className="text-slate-500 max-w-2xl">
          Start your Pre-RAG pipeline by uploading source documents. We support PDF, CSV, Markdown,
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="mt-6 flex gap-2">
            {['PDF', 'CSV', 'MD', 'TXT'].map((type) => (
              <span
                key={type}
                className="px-2 py-1 bg-slate-100 text-slate-500 text-xs font-bold rounded border border-slate-200 uppercase tracking-tighter"
              >
                {type}
              </span>
            ))}
