  ...
  return (
    <div className="fixed bottom-8 right-8 z-50 animate-in slide-in-from-right-12 fade-in duration-500 max-w-[320px] w-full">
      ...
      >
        ...
        <div
          className={`absolute -top-12 -right-12 w-32 h-32 blur-3xl rounded-full opacity-20 pointer-events-none
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="space-y-4">
            <p className="text-xs text-slate-300 leading-relaxed font-medium">{stage.text}</p>
            ...
            <div className="flex items-center gap-2 py-1">
              {steps.map((s, idx) => {
                ...
                const isCompleted =
                  isPast || (stepNum === 3 && stage.step === 3 && activeView === 'search');
                ...
                return (
                  <React.Fragment key={s.id}>
                    ...
                    {idx < steps.length - 1 && (
                      <div
                        className={`h-[1px] w-4 ${isCompleted ? 'bg-green-500' : 'bg-slate-800'}`}
                      />
                    )}
                  </React.Fragment>
                );
