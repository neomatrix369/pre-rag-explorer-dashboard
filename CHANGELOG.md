...
- **Project hardening**: `scripts/quality-gates.sh` (CI mirror), `.env.example`, `.editorconfig`, Dependabot, optional `.pre-commit-config.yaml`, commit-msg AI trailer stripper, contributor development guide, ADR-001
- **Cross-check phase 2**: `check_integrity.sh`, `print_slice_progress.sh`, import smoke tests, `.gitattributes`, `--full` quality gates, architecture + release docs, benchmarks scaffold
